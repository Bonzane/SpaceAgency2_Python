from enum import IntEnum, Enum


class PacketType(IntEnum):
    REQUEST_STEAM_INFO = 0x0000
    TCP_SERVER_TELL_CONNECTED_PLAYERS = 0x0001
    CHAT_MESSAGE_RELAY = 0x0002
    KEEPALIVE = 0x0003
    PLAYER_JOIN = 0x0004
    PLAYER_LEAVE = 0x0005
    INFO_ABOUT_PLAYERS = 0x0006
    INFO_ABOUT_AGENCIES = 0x0007
    CREATE_AGENCY = 0x0008
    AGENCY_DETAILS = 0x0009
    PLAYER_DETAILS = 0x000A
    LEAVE_AGENCY = 0x000B
    LIST_OF_AGENCIES = 0x000C
    JOIN_PUBLIC_AGENCY = 0x000D
    AGENCY_GAMESTATE = 0x000E
    GAME_JSON = 0x000F
    CONSTRUCT_BUILDING = 0x0010
    CONSTRUCT_VESSEL = 0x0011
    RESOLVE_VESSEL_REPLY = 0x0012
    VESSEL_CONTROL = 0x0013
    FORCE_RESOLVE_VESSEL = 0x0014
    UPGRADE_BUILDING = 0x0015
    SELL_RESOURCE = 0x0016
    CRAFT_RESOURCES = 0x0017
    AGENCY_INVITE = 0x0018
    ENTER_TERRAIN = 0x0019
    ENTER_TERRAIN_REPLY = 0x001A
    EXIT_TERRAIN = 0x001B
    EXIT_TERRAIN_REPLY = 0x001C


class DataGramPacketType(IntEnum):
    LATENCY_LEARN_PORT = 0x00
    PLAYER_DETAILS_UDP = 0x01
    UDP_ASK_ABOUT_AGENCY = 0x02
    OBJECT_STREAM = 0x03
    OBJECT_INQUIRY = 0x04
    RESOLVE_VESSEL = 0x05
    VESSEL_STREAM = 0x06
    REGION_CUE = 0x07
    TELESCOPE_SIGHT = 0x08
    NOTIFICATION = 0x09
    NOTIFY_VESSEL_DESTROYED = 0x0A
    VESSEL_UPGRADE_TREE = 0x0B
    REQUEST_VESSEL_TREE_UPGRADE = 0x0C
    BOARD_ASTRONAUT = 0x0D
    UNBOARD_ASTRONAUT = 0x0E
    CHANGE_ASTRONAUT_SUIT = 0x0F
    MAGNETOMETER_FIELD = 0x10
    GET_JETTISON = 0x11
    CARGO_ADD = 0x12
    CARGO_REMOVE = 0x13
    CARGO_STATE = 0x14
    SIGNAL_DESTROY = 0x15
    XP_ORB = 0x16
    CHANGE_ASTRONAUT_NAME = 0x17
    CAMERA_CONTEXT = 0x18
    CAMERA_CONTEXT_REPLY = 0x19
    REGION_NAME_REQUEST = 0x20
    RESOLVE_PLANET = 0x21
    ASTRONAUT_CONTROL_REQUEST = 0x22
    ASTRONAUT_CONTROL_REPLY = 0x23
    ASTRONAUT_COMMAND = 0x24
    ASTRONAUT_STREAM = 0x25


class ChatMessage(IntEnum):
    GLOBAL = 0x00
    AGENCY = 0x01
    DIRECTED = 0x02
    SERVERWARNING = 0x03
    SERVERCRITICAL = 0x04
    ACHIEVEMENT = 0x05
    ADMINISTRATOR = 0x06
    PLAYERJOIN = 0x07
    PLAYERLEAVE = 0x08
    SERVERGENERAL = 0x09
